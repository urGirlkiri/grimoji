  ...
  group('BehaviorRegister Tests', () {
    ...
    test('Should correctly identify and build mapped behaviors', () {
      for (final emoji in emojisWithBehaviors) {
        expect(
          BehaviorRegister.hasBehavior(emoji),
          isTrue,
    ...
    test('Should return null for normal emojis', () {
      expect(
        BehaviorRegister.hasBehavior(Emojis.rock),
        isFalse,
        reason: 'Emojis.rock should not have a registered behavior',
      );
      expect(
        BehaviorRegister.getBehaviorFor(Emojis.rock),
        isNull,
        reason: 'getBehaviorFor should return null for Emojis.rock',
      );

      expect(
        BehaviorRegister.hasBehavior(Emojis.droplet),
        isFalse,
        reason: 'Emojis.droplet should not have a registered behavior',
      );
      expect(
        BehaviorRegister.getBehaviorFor(Emojis.droplet),
        isNull,
    ...
    test(
      'All behaviors must safely handle every lifecycle event without crashing',
      () {
        for (final emoji in emojisWithBehaviors) {
          final behavior = BehaviorRegister.getBehaviorFor(emoji)!;
          ...
          expect(
            () => behavior.onBlastNearby(3, 3, ReactionType.explosive),
            returnsNormally,
          ...
          expect(
            ...
            isA<List<BehaviorAction>>(),
            reason:
                '$typeName onBlastNearby must return a List<BehaviorAction>',
          );

          expect(
            () => behavior.onSwipedWith(4, 4, Emojis.fire),
            returnsNormally,
