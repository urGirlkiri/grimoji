...
void main() {
  group('BoardManager Tests', () {
    ...
    test('Should initialize an 8x5 board with NO immediate matches', () {
      ...
      final initialMatches = MatchDetector.findMatchedGroups(
    ...
    test(
      'swapTiles should swap positions in the array AND update internal tile coordinates',
      () {
        ...
        expect(
          ...
          originalEmojiB,
          reason: 'Tile A should now be in Tile B\'s original position',
        );
        expect(
          boardManager.gridTiles[0][1].emoji,
          originalEmojiA,
          reason: 'Tile B should now be in Tile A\'s original position',
        );

        expect(
          boardManager.gridTiles[0][0].coordinate.col,
          0,
          reason:
              'Tile A\'s internal coordinate should update to its new position',
        );
        expect(
          boardManager.gridTiles[0][1].coordinate.col,
          1,
          reason:
              'Tile B\'s internal coordinate should update to its new position',
        );
      },
    );
    ...
    test(
      ...
      () {
        ...
        expect(
          boardManager.gridTiles[3][3].coordinate.row,
          3,
          reason:
              'coordinate.row should snap back to its actual row after falling',
        );
      },
    );
    ...
    group('Adjacency Finders', () {
      test('findAdjacentFilledTile should find a neighbor', () {
        ...
        expect(
          dx + dy,
          1,
          reason: 'Neighbor must be exactly 1 step away (no diagonals)',
        );
      });
      ...
      test(
        'findAdjacentEmptyTile should return a coordinate when an empty tile exists',
        () {
          const emptyEmoji = GameEmoji(
            'svg/empty.svg',
            'lottie/empty.json',
            '',
            [],
          ...
          expect(
            emptySpace,
            isNotNull,
            reason: 'Should find the empty tile we just placed',
          );
    ...
    group('Flying Tiles', () {
      test('collectFlyingTiles should return false if nothing is flying', () {
        ...
        expect(
          result,
          isFalse,
          reason: 'Should return false when no tiles are isFlying',
        );
      });

      test(
        'collectFlyingTiles should return true and trigger gravity when tiles are flying',
        () {
          ...
          expect(
            result,
            isTrue,
            reason: 'Should return true because it found a flying tile',
          );
          expect(
            boardManager.gridTiles[7][2].isFlying,
            isFalse,
            reason: 'Tile should no longer be flying after collection',
          );
          expect(
            boardManager.gridTiles[7][2].emoji,
            equals(tileAbove),
            reason:
                'Gravity should have pulled the tile above down into the empty space',
          );
        },
      );
    });
    ...
    group('Swap & Memory Tests', () {
      test(
        'swapTiles correctly moves tiles and updates coordinates via copyWith',
        () {
          ...
          expect(
            newTileA.emoji,
            Emojis.fire,
            reason: 'Tile A did not move to column 1',
          );
          expect(
            newTileB.emoji,
            Emojis.droplet,
            reason: 'Tile B did not move to column 0',
          );

          expect(
            newTileA.hashCode,
            isNot(equals(hashA)),
            reason: 'Tile A was mutated instead of copied!',
          );
          expect(
            newTileB.hashCode,
            isNot(equals(hashB)),
            reason: 'Tile B was mutated instead of copied!',
          );

          expect(
            newTileA.id,
            equals(idA),
            reason: 'Tile A lost its unique ID during copyWith',
          );
          expect(
            newTileB.id,
            equals(idB),
            reason: 'Tile B lost its unique ID during copyWith',
          );
    ...
    group('Flag Management', () {
      test('flagFlyingTargetEmojis should flag target emoji tiles', () {
        ...
        expect(
          boardManager.gridTiles[0][0].isFlying,
          isTrue,
          reason: 'Target emoji should be flagged as flying',
        );
        expect(
          boardManager.gridTiles[1][1].isFlying,
          isFalse,
          reason: 'Non-target emoji should not be flagged',
        );
      });
    ...
    group('Shuffle and Triggered', () {
      ...
      test('getTriggeredEmojis should return only triggered tiles', () {
        ...
        expect(
          triggered.any((t) => t.emoji == boardManager.gridTiles[0][0].emoji),
          isTrue,
        );
        expect(
          triggered.any((t) => t.emoji == boardManager.gridTiles[1][1].emoji),
          isTrue,
        );
      });

      test(
        'getTriggeredEmojis should return empty list when no tiles are triggered',
        () {
          final triggered = boardManager.getTriggeredEmojis();
    ...
    group('Get Adjacent Tiles', () {
      test('getAdjacentTiles should return correct neighbors', () {
        ...
        expect(
          adjacent.length,
          4,
          reason: 'Center tile should have 4 neighbors',
        );

        final positions = adjacent
      ...
      test('getAdjacentTiles should handle edge cases', () {
        ...
        expect(
          adjacent.length,
          2,
          reason: 'Corner tile should have 2 neighbors',
        );
      });
    });
