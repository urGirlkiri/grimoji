...
void main() {
  group('BehaviorEngine Tests', () {
    ...
    test(
      'initializeBehavior should assign behavior if it exists, and do nothing if null',
      () {
        final bugTile = Tile(
        ...
        expect(
          bugTile.behavior,
          isNotNull,
          reason: 'Bug should get the SpyBehavior assigned',
        );
        expect(
          rockTile.behavior,
          isNull,
          reason: 'Rock should have no behavior',
        );
      },
    );
    ...
    group('Event Triggers', () {
      test(
        'processMatchedBehavior safely handles tiles with and without behaviors',
        () {
          final bugTile = Tile(
            coordinate: TileCoordinate(row: 0, col: 0),
            emoji: Emojis.bug,
          ...
          expect(
            spyBehavior.matchedCalled,
            isTrue,
            reason: 'Engine should trigger onMatched',
          );
          ...
          expect(
            () => behaviorEngine.processMatchedBehavior(rockTile, 0, 0),
            returnsNormally,
      ...
      test(
        'processBlastBehavior safely handles tiles with and without behaviors',
        () {
          final bugTile = Tile(
            coordinate: TileCoordinate(row: 0, col: 0),
            emoji: Emojis.bug,
          ...
          behaviorEngine.processBlastBehavior(
            bugTile,
            0,
            0,
          ...
          expect(
            spyBehavior.blastCalled,
            isTrue,
            reason: 'Engine should trigger onBlastNearby',
          );
      ...
      test(
        'processSwipedWithBehavior returns actions if behavior exists, else empty list',
        () {
          final bugTile = Tile(
            coordinate: TileCoordinate(row: 0, col: 0),
            emoji: Emojis.bug,
          ...
          final actions = behaviorEngine.processSwipedWithBehavior(
            bugTile,
            0,
            0,
            Emojis.fire,
          ...
          expect(
            spyBehavior.swipedCalled,
            isTrue,
            reason: 'Engine should trigger onSwipedWith',
          );
          expect(
            actions.length,
            1,
            reason: 'Should return the actions provided by the behavior',
          );
          expect(
            actions.first.type,
            ActionType.doNothing,
            reason:
                'Returned action should be the one defined in the SpyBehavior',
          );
          ...
          final noActions = behaviorEngine.processSwipedWithBehavior(
            rockTile,
            0,
            0,
            Emojis.fire,
          ...
          expect(
            noActions,
            isEmpty,
            reason: 'Normal tiles should return an empty action list',
          );
        },
      );
    });
    ...
    group('Action Execution Logic', () {
      test(
        'ActionType.placeEmoji should clone the emoji into an adjacent empty tile',
        () {
          ...
          expect(
            neighbors.contains(Emojis.bug),
            isTrue,
            reason: 'Engine should place a new Bug',
          );
        },
      );

      test(
        'ActionType.reactEmoji should overwrite a filled neighbor and clear its behavior',
        () {
          boardManager.gridTiles[1][1].emoji = Emojis.bug;
          ...
          final transmutedTiles = neighbors
              .where((n) => n.emoji == Emojis.cloud)
              .toList();
          expect(
            transmutedTiles.length,
            1,
            reason: 'One of the neighbor rocks should have become a Cloud',
          );
          expect(
            transmutedTiles.first.behavior,
            isNull,
            reason: 'The transmuted tile must have its behavior wiped',
          );
        },
      );
      ...
      test('Should safely handle ActionType.doNothing', () {
        ...
        expect(
          () => behaviorEngine.executeBehaviorActions(actions, 0, 0),
          returnsNormally,
      ...
      test(
        'Should safely ignore invalid actions (null emojis or no available targets)',
        () {
          final nullEmojiAction = [
          ...
          expect(
            () => behaviorEngine.executeBehaviorActions(nullEmojiAction, 1, 1),
            returnsNormally,
            reason:
                'Engine should safely ignore actions with null emojis without error',
          );
          ...
          expect(
            () => behaviorEngine.executeBehaviorActions(placeAction, 1, 1),
            returnsNormally,
            reason:
                'Engine should safely execute valid placeEmoji actions without error',
          );
          ...
          expect(
            ...
            returnsNormally,
            reason:
                'Engine should safely ignore transmute actions with no available targets without error',
          );
        },
      );
    });
